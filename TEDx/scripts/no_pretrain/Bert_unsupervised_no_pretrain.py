...
if __name__ == '__main__':
	...
	df_with_ids = pd.read_csv(test_set, sep="\t")
	...
	print("Configurazione modello originale: {}".format(model.config))
	model = randomize_model(model)
	
	print("Configurazione modello modificato: {}".format(model.config))
	...
	#save_dataframe("{}_cosine_distance_no_pretrain".format(args.name), df_cosine_distance, path_cosine_distance)
	save_dataframe("{}_cosine_distance_no_pretrain".format(args.name), df_cosine_distance, path_results)

	#salvo il dizionario con id_evento - distanza cosenica per ogni livello
	# with open('{}{}_diz_ids_cosine_distances_no_pretrain.pickle'.format(path_pickle, args.name), 'wb') as handle:
	# 	pickle.dump(diz_ids_cosine_distances, handle)
	with open('{}{}_diz_ids_cosine_distances_no_pretrain.pickle'.format(path_results, args.name), 'wb') as handle:
		pickle.dump(diz_ids_cosine_distances, handle)
