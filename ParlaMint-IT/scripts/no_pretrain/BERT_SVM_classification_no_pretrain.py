...
def train_and_predict(layer, X_train, X_test, y_train, y_test, ids_train, ids_test):
	...
	model = SVC(kernel='linear', gamma='auto', C=1.0)
...
if __name__ == '__main__':
	...
	path_risorse = "./results"
	# path_salvataggio = "../pickle"
	# path_salvataggio = "./"
	path_salvataggio = "./results"
	...
	for layer in tqdm(list(range(1,13))):
		...
		train_and_predict(layer, X_train, X_test, y_train, y_test, ids_train, ids_test)

	diz_classifiers_df = {}
	diz_classifiers_df[df_name] = diz_classifiers
	
	with open(path_salvataggio+'/'+'diz_classifiers_bert_svm_{}_no_pretrain.pickle'.format(df_name), 'wb') as handle:
